...
for k=1:nbin
   ...
   mpofw(k+1,:)=mpofw(k,:);
    for jt=1:nt 
     %apply odd 2q terms
    for j=1:2:n-1
        [mpofw{k+1,j},mpofw{k+1,j+1}]=gate_2q(mpofw{k+1,j},mpofw{k+1,j+1},g2{j},sv_min,D);
    end
     %apply even 2q terms
    for j=2:2:n-1
        [mpofw{k+1,j},mpofw{k+1,j+1}]=gate_2q(mpofw{k+1,j},mpofw{k+1,j+1},g2{j},sv_min,D);
    ...
    %apply 1q terms
    for j=1:n
        [mpofw{k+1,j}]=gate_1q(mpofw{k+1,j},g1{j});
    end
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
g1=cell(1,n);
   ...
   mpobw(k+1,:)=mpobw(k,:);
    %apply 1q terms
    for j=1:n
        [mpobw{k+1,j}]=gate_1q(mpobw{k+1,j},g1{j});
    end
    for jt=1:nt
    %apply even 2q terms
    for j=2:2:n-1
        [mpobw{k+1,j},mpobw{k+1,j+1}]=gate_2q(mpobw{k+1,j},mpobw{k+1,j+1},g2bw{j},sv_min,D);
    end
    %apply odd  2q terms
    for j=1:2:n-1
        [mpobw{k+1,j},mpobw{k+1,j+1}]=gate_2q(mpobw{k+1,j},mpobw{k+1,j+1},g2bw{j},sv_min,D);
...
for k=1:nbin
    ...
    ovl_temp=mpo_overlap(tnsbw_temp,tnsfw_temp);
    for jc=1:nc  
            ovl_diff_left=0;
           for js=1:length(Hc(jc).sys)
                 jq=Hc(jc).sys(js);
            ...
            ovl_diff_left=ovl_diff_left+mpo_overlap(tnsbw_temp,tnsfw_diff_left);
              ...
              iG(k,jc)=-2*real(ovl_diff_left*conj(ovl_temp));
    end
end
