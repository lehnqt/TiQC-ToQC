function [iF,iG]=infid(H0,Hc,c,T,mpo0,mpotg,sv_min,D,Dc,nsweep,midstep,nt,iscpr,iso)
n=length(mpo0);
nc=length(Hc);
nbin=length(c)/(nc);
c=reshape(c,[nbin,nc]);
...
if mod(k-1,midstep)==0||k==nbin
    if iscpr==1
     mpofw(k+1,:)=mpo_recompress(mpofw(k+1,:),sv_min,Dc,nsweep);
     mpobw(k+1,:)=mpo_recompress(mpobw(k+1,:),sv_min,Dc,nsweep);
    end
